...
html_markdown <- function(node, split_tags = NULL, trim_splits = TRUE, omit_empty_splits = TRUE, convert_table = TRUE) {
  ...
  markdown <- function(node, text = TRUE, trim = FALSE) {
    text <- switch(
      ...
      # squash + trim for <p>?
      p = { stri_c(markdown_contents(node, trim = TRUE), "\n\n") },
      a = {
      ...
      div = markdown_contents(node, text = FALSE, trim = TRUE) |> stri_c("\n\n"),
      section = markdown_contents(node, text = FALSE, trim = TRUE) |> stri_c("\n\n"),
      # span = ,
      # section = ,
