...
#' sensible boundary (heading, paragraph, sentence, word, or character). It
#' returns a tibble recording the character ranges of each chunk and, if
...
#'   between successive chunks. Default: `0.5`. Even when `0`, small overlaps
...
#'   between target chunk start. Chunks that end up on identical boundaries are
#'   merged.
#' @param ... Must be empty.
...
#' markdown_chunk(md, target_size = 40, target_overlap = 0, max_snap_dist = 40)
...
markdown_chunk <- function(
  ...
  for (boundary_type in c("heading", "paragraph", "sentence", "word", "char")) {
  ...
  chunks <- chunk_targets |>
    ...
    # the larger chunk
    slice_max(end, by = start, with_ties = FALSE, n = 1) |>
    slice_min(start, by = end, with_ties = FALSE, n = 1) |>
    arrange(start)
...
markdown_position_headings <- function(
  ...
  # TODO: still need to figure out what to do if the chunk starts in the middle of a
  # heading, e.g., for pathological inputs with an extremely long headings or
  # max_snap_dist==0
  # browser()
  left_join(
