...
This approach presents retrieved content directly, and maintaining a
defined structure helps prevent the LLM from confusing retrieved content
with user queries. The row-oriented format also ensures that all
metadata (such as headings) stays attached to each row, so the context
for each chunk appears next to its content.
