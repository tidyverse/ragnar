...
If your documents are already markdown files, you can turn them into
ragnar MarkdownDocument`objects with the`MarkdownDocument()\` function.
...
You can define chunk boundaries with `markdown_chunk()`'s
`segment_by_heading_levels` argument, which takes a vector of integers
between 1 and 6. Chunk text will not overlap a defined segment boundary.
...
rag_retrieve_quarto_excerpts <- local({
  ...
  function(text) {
    ...
    chunks <- ragnar::ragnar_retrieve(
      text,
      top_k = 10,
      filter = !.data$id %in% retrieved_chunk_ids
    )

    # Update seen chunks
    retrieved_chunk_ids <<- unique(unlist(c(retrieved_chunk_ids, chunks$id)))
...
To see the complete structure of a store, `tbl(store@con, "chunks")`
offers a high-level SQL view of the data and
`tbl(store@con, "chunks") |> collect()` creates a tibble.
