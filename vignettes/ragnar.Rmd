...
improving large language model (LLM) outputs by grounding them in
...
documentation using `ragnar`. The examples are simplified for clarity;
...
RAG addresses this by retrieving relevant excerpts from a trusted corpus
of vetted sources and asking the LLM to summarize, paraphrase, or answer
the user's question using only that material. This grounds the response
in known content and reduces the risk of hallucination. RAG shifts the
LLM's job from open-ended generation to summarizing or quoting from
retrieved material.
...
it's helpful for RAG-based tools to present links back to the original
...
search, users spend time clicking, reading, and skimming, and may still
miss the answer.
...
You can use OpenAI, an open-source model via `embed_ollama()`, or your
own function.
...
which can accept a wide variety of formats (pdf, docx, pptx, html). In
many cases it works well, but for specialized needs you can opt for a
...
Chunking is delicate; each chunk should stand alone without relying on
the context of the surrounding document. Ideally, we can split the text
at natural points like headings, paragraphs, or sentences, and avoid
splits in the middle of a sentence or word.

Additionally, we augment chunks with context that describes the chunk’s
origin—such as URL, title, headings, and subheadings—both so the LLM can
provide links back to the source, and so the LLM and embedding models
can better situate the chunk’s content.

To help with these tasks, use `ragnar_read()` and `ragnar_chunk()`.

`ragnar_read()` can split a markdown document using markdown headings,
while also extracting the heading titles as columns in the dataframe.
Use the `frame_by_tags` argument and select the document headings you
want to segment by. If you’re starting from already converted markdown
...
## Retrieval and System Prompt Design
...
completions, like any other LLM output. We minimize the complexity in
the tool interface to minimize the chance for LLM errors.
...
This provides some slack, so the system can handle less-than-perfect
search results or unclear questions.
...
spend effort optimizing, and it's helpful to test in the context of an
end-to-end application.

You can use `ragnar_store_inspect()` to interactively see what kind of
...
If the results shown in the inspector do not seem useful—either because
they lack sufficient context or are chunked in a way that fails to
preserve meaning—they likely won't be useful to an LLM either. Iterate
on the store creation pipeline until retrieval returns meaningful
excerpts.
...
place in the docs, with enough information to decide if you need to read
further. We've designed it to escalate gracefully: if the LLM is not
able to provide an accurate answer, the user can transition to reading
the source material without friction.
...
-   The cost of generating embeddings is negligible. For example,
    generating embeddings for the entire Quarto website costs less than
    a cent.
...
grounded answers—while always giving users a path to the original
source.
