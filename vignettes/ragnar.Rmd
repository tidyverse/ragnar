...
it's still helpful for RAG-based tools to present links back to the
original material so users can check context and verify details.
...
the text at natural points like headings, paragraphs, or sentences, and
avoid splits in the middle of a sentence or word.
...
complexity in the tool interface to minimize the chance for LLM errors.
...
they need to read further. It's designed it to escalate on failure
gracefully: if the LLM is not able to provide an accurate answer, the
user can transition to reading the source material without friction.
